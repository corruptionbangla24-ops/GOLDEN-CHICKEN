...
// 🐓 ৫-রিলস ওরিজিনাল গোল্ডেন চিকেন ক্যাসিনো র্যান্ডমাইজেশন প্রতীক তালিকা ভাই ভাই
const goldenChickenPool = ["GOLDEN_CHICKEN", "GOLDEN_EGG", "MONEY_BAG", "LUCKY_SYCEE", "CHICK_ICON", "CORN_GOLD", "NEST_COINS"];
...
// 🛫 ২. গোল্ডেন চিকেন ৫-রিল কোর স্লট স্পিন রাউট (POST Route - ৯৫% RTP গাণিতিক বর্ম কঠোর লক ভাই ভাই!)
app.post('/api/chicken-spin', async (req, res) => {
    ...
    try {
        ...
        let adminTriggeredPrize = (balResponse.data && balResponse.data.chicken_target) ? balResponse.data.chicken_target : null;

        let spinResults, finalStatus, winMultiplier;
        let isLoopActive = true;
        let loopSafety = 0;

        // 🎰 [🎰 ৯৫% ওরিজিনাল ক্যাসিনো RTP ও ৫-রিলস ফরচুন এগ গাণিতিক লুপ ভাই ভাই]
        while (isLoopActive && loopSafety < 200) {
            ...
            spinResults = [];
            for (let r = 0; r < 5; r++) {
                spinResults.push(goldenChickenPool[Math.floor(Math.random() * goldenChickenPool.length)]);
            }
            ...
            if (maxSameSymbolsCount >= 3) {
                finalStatus = "win";
                // 🚀 [ডাইনামিক কম্বো মাল্টিপ্লায়ার ওডস]: ৩টি মিললে ৫.০০ গুণ, ৪টি মিললে ৩৫ গুণ, ৫টি মিললে ৩০০ গুণের গ্র্যান্ড ফরচুন জ্যাকপট!
                if (maxSameSymbolsCount === 3) winMultiplier = 5.00;
                else if (maxSameSymbolsCount === 4) winMultiplier = 35.00;
                else winMultiplier = 300.00; 
            } else {
                finalStatus = "lose";
                winMultiplier = 0.00;
            }
            ...
            if (adminTriggeredPrize) {
                ...
                if (adminTriggeredPrize === "force_jackpot" && winMultiplier === 300.00) isLoopActive = false;
            } else {
                // ৩০০ গুণের মেগা জ্যাকপটের চান্স আরটিপি লুপ ট্র্যাকে কড়া সুরক্ষায় টাইট ০.২% এ লক ভাই ভাই
                if (winMultiplier === 300.00 && Math.random() > 0.002) continue;
        ...
        console.error("Golden Chicken Core Engine Error:", e.message);
...
io.on('connection', (socket) => { console.log("Player connected to Royal Golden Chicken 5-Reel Slot Engine!"); });

// গোল্ডেন চিকেন গেম নিজস্ব কাস্টম ৫৮০০ পোর্টে কড়া নিয়নে অন ফায়ার ভাই ভাই!
const PORT = process.env.PORT || 5800; 
server.listen(PORT, () => { console.log(`🎡 Royal Golden Chicken Engine Running on port ${PORT}`); });
