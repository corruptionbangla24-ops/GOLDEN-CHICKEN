...
// 🐓 ওরিজিনাল গোল্ডেন চিকেন ৫টি ফরচুন ক্যাসিনো সিম্বল ম্যাপ
const chickenSymbolsPool = ["CHICKEN", "GOLD_EGG", "NEST", "CHICK_CROW", "FEATHER"];
...
app.get('/api/slot-balance', async (req, res) => {
    ...
    try {
        const response = await axios.post(`${MAIN_SITE_URL}/api_callback.php`, {
            action: "balance", username: finalUser, amount: 0, wallet: targetWallet, game: "goldenchicken"
        }, { timeout: 15000 });
...
// 🛫 ২. গোল্ডেন চিকেন কোর ৫-রিল স্পিন রাউট (মানি ট্রি ও ফ্যান-টানের মতো ১০০% সিকিউরড সিঙ্গেল পাইপলাইন প্রোটোকল)
app.post('/api/slot-spin', async (req, res) => {
    const { userId, amount, wallet } = req.body; 
    const reqAmount = parseFloat(amount) || 50;
    const finalGameName = "goldenchicken"; 
    const targetWallet = wallet || "main";
    ...
    try {
        ...
        while (isLoopActive && loopSafety < 150) {
            ...
            // ৫টি রিলের জন্য ৫টি পিউর র্যান্ডম গোল্ডেন ফরচুন সিম্বল সিলেকশন লক চ্যাম
            for (let i = 0; i < 5; i++) {
                let randomIdx = Math.floor(Math.random() * chickenSymbolsPool.length);
                finalReelsResultMatrix.push(chickenSymbolsPool[randomIdx]);
            }
            ...
            if (maxMatchesCount === 5) {
                ...
                if (matchedSymbolName === "CHICKEN") winMultiplier = 50.00;       
                else if (matchedSymbolName === "GOLD_EGG") winMultiplier = 25.00; 
                else if (matchedSymbolName === "NEST") winMultiplier = 20.00; 
                else if (matchedSymbolName === "CHICK_CROW") winMultiplier = 15.00; 
                else winMultiplier = 12.00;                          
                finalStatus = "win";
            } else if (maxMatchesCount === 4) {
                winMultiplier = (matchedSymbolName === "CHICKEN") ? 8.00 : 4.00;
                finalStatus = "win";
            } else if (maxMatchesCount === 3) {
                winMultiplier = (matchedSymbolName === "CHICKEN") ? 2.50 : 1.50;
                finalStatus = "win";
            ...
            if (balResponse.data && balResponse.data.goldenchicken_target) {
                let target = String(balResponse.data.goldenchicken_target).toUpperCase();
                if (target === "FORCE_LOSE" && finalStatus === "win") {
                    finalReelsResultMatrix = ["CHICKEN", "GOLD_EGG", "NEST", "CHICK_CROW", "FEATHER"];
...
server.listen(PORT, () => { console.log(`💎 Golden Chicken Fortune Eggs Slots Engine Running on port ${PORT}`); });
